...
| `INGEST_DISPATCH_RETRY_BACKOFF_SECONDS` | `1` | `1` | Float `>= 0`. | Reserved retry backoff interval for dispatcher retries. |
...
| `INGEST_EMBEDDING_ELASTIC_MAPPING_VERSION` | `v1` | `v2` | `v1`, `dense_vector`, `dense_vector_v1`, `v2`, `semantic_text`, or `semantic_text_v2`. | Dispatcher output schema. `v1` embeds `content` and `title` through a dense-vector ingest pipeline. `v2` uses the index default pipeline to copy `content` and `title` into `semantic_text` fields for inference, while `_source` excludes those duplicate semantic fields. |
...
| `INGEST_EMBEDDING_ELASTIC_INCLUDE_LOCAL_PATHS` | `false` | `false` | `true` or `false`. | Compatibility flag. The current lean Elastic payload does not send local filesystem paths; local paths remain only in local artifacts and job metadata. |
...
| `INGEST_DOCLING_PDF_PICTURE_DESCRIPTION_RUNTIME` | `transformers` | `remote_llm` | `auto`, `auto_inline`, `transformers`, `remote_llm`, or aliases `remote`, `api`, `vllm`. | Runtime requested for standard-pipeline picture descriptions. The legacy value `vllm` is accepted as an alias for `remote_llm`. |
...
| `INGEST_DOCLING_VLM_RUNTIME` | `transformers` | `remote_llm` | `auto`, `auto_inline`, `transformers`, `remote_llm`, or aliases `remote`, `api`, `vllm`. | Runtime requested for full-page VLM conversion. The legacy value `vllm` is accepted as an alias for `remote_llm`. |
...
| `INGEST_DOCLING_VLM_TRUST_REMOTE_CODE` | unset | `true` | Unset, `true`, or `false`. | Allows custom Hugging Face model code for VLM loaders. Enable only after inspecting and trusting the model repository. Overrides the legacy vLLM-specific setting when set. |
...
## Legacy vLLM Compatibility Variables

These variables remain in the settings model so existing `.env` files continue
to load. The production runtime no longer passes them to Docling's in-process
vLLM engine; use `remote_llm` and configure the external inference server
instead.

| Variable | Code Default | Example | Allowed Values | Explanation |
| --- | --- | --- | --- | --- |
| `INGEST_DOCLING_VLLM_TENSOR_PARALLEL_SIZE` | `1` | `1` | Integer `>= 1`. | Legacy compatibility value. Configure tensor parallelism on the external `vllm serve` process instead. |
| `INGEST_DOCLING_VLLM_GPU_MEMORY_UTILIZATION` | `0.9` | `0.9` | Float `> 0` and `<= 1`. | Legacy compatibility value. Configure GPU memory utilization on the external inference server instead. |
| `INGEST_DOCLING_VLLM_TRUST_REMOTE_CODE` | `false` | `false` | `true` or `false`. | Backward-compatible alias for remote-code trust. Prefer `INGEST_DOCLING_VLM_TRUST_REMOTE_CODE` for local Transformers loading and configure remote-code trust on the external inference server. |
| `INGEST_DOCLING_VLLM_CUDAGRAPH_MODE` | `PIECEWISE` | `PIECEWISE` | `NONE`, `FULL`, `PIECEWISE`, `FULL_AND_PIECEWISE`, or `FULL_DECODE_ONLY`. | Legacy compatibility value. Configure CUDA graph behavior on the external inference server instead. |
| `INGEST_DOCLING_VLLM_MODEL_IMPL` | `auto` | `auto` | Any string accepted by the external vLLM server, typically `auto` or `transformers`. | Legacy compatibility value. Configure model implementation on the external inference server instead. |
| `INGEST_DOCLING_VLLM_ENFORCE_EAGER` | unset | `true` | Unset, `true`, or `false`. | Legacy compatibility value. Configure eager mode on the external inference server instead. |
| `INGEST_DOCLING_VLLM_MAX_MODEL_LEN` | unset | `32768` | Unset or integer `>= 1`. | Legacy compatibility value. Configure max model length on the external inference server instead. |
| `INGEST_DOCLING_VLLM_MAX_NUM_BATCHED_TOKENS` | unset | `8192` | Unset or integer `>= 1`. | Legacy compatibility value. Configure batched token limits on the external inference server instead. |
| `INGEST_DOCLING_VLLM_FALLBACK_RUNTIME` | `transformers` | `transformers` | `auto_inline` or `transformers`. | Legacy compatibility value. `vllm` runtime requests are now normalized to `remote_llm`. |
| `INGEST_DOCLING_VLLM_FALLBACK_ON_UNSUPPORTED` | `true` | `true` | `true` or `false`. | Legacy compatibility value. RemoteLLM endpoint compatibility is validated by the external server and optional health check. |
| `INGEST_DOCLING_VLLM_ALLOW_UNVERIFIED_MODELS` | `false` | `false` | `true` or `false`. | Legacy compatibility value. Serve experimental models through a trusted external endpoint. |
