...
| `INGEST_CHUNK_TOKENIZER_PATH` | unset | `/datastore/models/tokenizers/qwen3-embedding-4b` | Local filesystem path. | Local Hugging Face tokenizer directory for token chunking. Runtime tokenizer downloads are not used by default. |
...
| `INGEST_EMBEDDING_ELASTIC_INDEX` | `ingest-embedding-input` | `open-rag-embeddings-v3` | Non-empty Elasticsearch index name. | Target Elasticsearch index for RAG records sent through the official bulk helper. |
| `INGEST_EMBEDDING_ELASTIC_MAPPING_VERSION` | `v1` | `v3` | `v1`, `dense_vector`, `dense_vector_v1`, `v2`, `semantic_text`, `semantic_text_v2`, `v3`, or `semantic_text_v3`. | Dispatcher output schema. `v1` embeds `content` and `title` through a dense-vector ingest pipeline. `v2` keeps the legacy duplicated `content_semantic`/`title_semantic` semantic-text fields. `v3` stores canonical chunk text once in `content`, derives language-routed `content_lex.*` fields in the index default pipeline, and excludes those lexical copies from `_source`. |
| `INGEST_EMBEDDING_ELASTIC_PIPELINE` | unset | `qwen3_embeddings_pipeline` | Unset or any Elasticsearch ingest pipeline name. | Optional Elasticsearch ingest pipeline name for dispatcher bulk actions. Use this for v1 dense-vector embedding fields. Leave blank for v2/v3 because their index assets set `index.default_pipeline`. |
