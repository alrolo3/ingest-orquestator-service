...
| `INGEST_EMBEDDING_ELASTIC_MAPPING_VERSION` | `v1` | `v2` | Dispatcher output schema. `v1` sends `content` and `title` through a dense-vector ingest pipeline. `v2` sends only `content` and `title`; the index default pipeline copies those values into `content_semantic` and `title_semantic` for `semantic_text` inference. Aliases: `dense_vector_v1`, `semantic_text_v2`. |
