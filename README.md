...
the current plugin dependency range, and the upper bound avoids a SuryaOCR
incompatibility in Transformers 5.x. For local development on another platform, set
...
always enabled, defaults to English, and accepts request-specific language
selection through the `ocr_languages` API query parameter.
